...
- **UI Label (form section):** "Solution Notes"
- **Field Name (ADO settings):** "Technical Solution"
- **API reference name:** Needs confirmation -- likely `Custom.TechnicalSolution` or `Microsoft.VSTS.CMMI.TechnicalSolution`

To confirm, we'll query `/_apis/wit/fields` filtered by name, or inspect a raw `get_user_story` response for a US that has this field populated. Once confirmed, add the reference name to `US_FIELDS` in [src/tools/work-items.ts](src/tools/work-items.ts).
