...
export function registerAllPrompts(server: McpServer) {
    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "   - title, useCaseSummary (title-summary-only), description/prerequisites, steps, priority (1-4), state, assignedTo, areaPath, iterationPath.",
          "   - Ask: 'Which field(s) should I update? Reply with the field name(s).' Wait for an exact answer.",
          "",
          "   **Title-update guidance.** If the user wants to change the test case title, FIRST decide which path applies — these three are mutually exclusive:",
          "   - **`useCaseSummary` (preferred for the common case).** When the user only wants to refine the trailing description (e.g. 'change the summary to ...', 'make the title clearer about X'), pass `useCaseSummary: '<their text>'`. The server fetches each TC, parses its existing title, and reconstructs the full title preserving the TC ID prefix, feature tags, and category tag. This is the safe path — it guarantees the `TC_<usId>(_<TAG>)?_<NN> -> ...` shape stays intact.",
          "   - **`title` (full structured title).** When the user supplies a complete title in the canonical shape (`TC_<usId>(_<TAG>)?_<NN> -> <feature-tags> -> <use-case-summary>`), pass it via `title`. The server validates the new value still parses; if it doesn't but the existing title does, it BLOCKS with `tc-title-shape-mismatch` so the user can pick an option (see step 8).",
          "   - **`forceTitleOverwrite: true` (escape hatch).** Power-user override — write the supplied `title` exactly as given without any shape validation. Only use when the user explicitly says they want to break the convention (e.g. cleaning up legacy titles).",
          "   - NEVER pass both `title` AND `useCaseSummary` — the tool returns an error.",
          "",
          ...
          "",
          "8. **If the tool returns `reason: \"tc-title-shape-mismatch\"`** (🚫 BLOCK — title-preservation gate): the user supplied a `title` that doesn't follow the `TC_<usId>(_<TAG>)?_<NN> -> ...` convention used by the existing TC(s). Show the `message`, `existingTitles`, and `newTitleProvided` verbatim, then list the three `options` (A / B / C). Apply the option selection contract — bare 'yes' is NOT a pick. On **A**: ask the user for the new use-case summary text only, then re-run with `useCaseSummary: '<that text>'` (drop `title`). On **B**: ask the user for the full structured title, then re-run with `title: '<that text>'`. On **C**: re-run with `title: '<original text>', forceTitleOverwrite: true` — only after the user explicitly chose C. Never pass `forceTitleOverwrite: true` without an explicit C.",
          "",
          "   Related responses you may also see:",
          "   - **`reason: \"use-case-summary-unparseable-existing-title\"`** (🚫 BLOCK): user passed `useCaseSummary` but at least one TC's existing title isn't in the canonical shape, so the server can't reconstruct from a prefix. Show the `unparseableTcs` list. Ask the user to provide the full structured `title` instead, OR to call out a smaller subset of TCs whose titles do follow the convention.",
          "   - **`reason: \"title-and-use-case-summary-both-supplied\"`** (🚫 BLOCK): both args were sent in the same call (this should not happen if step 3 was followed). Pick one path with the user and re-run.",
          "",
          "9. **If the tool returns `reason: \"precheck-failed\"`** (🚫 BLOCK): some IDs are not Test Cases or couldn't be fetched. Show the `typeRefusals` and `fetchFailures` tables verbatim. No changes were made. Ask the user to supply a corrected ID list, then re-run — do NOT call the tool with the filtered list on your own.",
          "",
          "10. **On success:**",
          ...
          "",
          "11. **For the two-group path:** after the first call succeeds, immediately call qa_tc_update again with the second group's IDs and values. If the first call fails or partially fails, STOP — do not call the second. Surface the failure and ask the user how to proceed.",
          "",
          "**Universal rules:**",
          ...
          "- NEVER pass `forceTitleOverwrite: true` without an explicit option-C reply to a `tc-title-shape-mismatch` block (or an explicit user request to break the convention).",
  ...
  server.registerPrompt("qa-workitem-enrich", {
    title: "Enrich Test Cases — Add Comment and/or Copy Attachments",
    description: "Add a comment and/or copy attachments from a source User Story to one or more linked test cases — single confirmation for the whole package",
  }, async () => ({
    messages: [{
      role: "user" as const,
      content: {
        type: "text" as const,
        text: [
          "I want to enrich existing test case(s) with a comment, attachments copied from another work item, or both.",
          "",
          "**Scope.** This command orchestrates two underlying tools — `qa_tc_comment_add` and `qa_tc_attachments_copy` — behind one flow. Either, or both, may run depending on what the user provides. The user confirms ONCE for the whole package; do not ask for a separate yes per tool.",
          "",
          "Please:",
          "",
          "1. **Identify the targets.** Ask: 'Which test case(s)? Reply with one or more IDs (e.g. `12345`, `12345, 67890`, `1200-1205`), OR a parent User Story ID and I'll look up the linked TCs.' If the user gives a User Story ID, call `qa_tests` with that ID to enumerate linked TCs. Show the result and confirm the target list before proceeding.",
          "",
          "2. **Identify the enrichment.** Ask the user, in this order:",
          "   - 'Add a comment? Paste the HTML/text, or reply **skip**.'",
          "   - 'Copy attachments? Reply with the source work item ID (e.g. the parent US), or **skip**. If you want only specific filenames, list them after the ID (e.g. `1345753 screenshot.png, spec.pdf`).'",
          "   At least ONE of comment or attachments must be provided. If both are skipped, stop and tell the user there's nothing to do.",
          "",
          "3. **Show the upfront plan and ask for ONE confirmation.** Render the plan compactly:",
          "",
          "   ```",
          "   I'll enrich N test case(s):",
          "     • [ID](url) — title",
          "     • ...",
          "",
          "   Comment to add: <first line of the comment, truncated to ~80 chars> (or 'none')",
          "   Attachments to copy: M file(s) from source #<id> (or 'none')",
          "     - filename1",
          "     - filename2",
          "",
          "   Reply **YES** to proceed, **no** to cancel, or tell me what you'd like to change.",
          "   ```",
          "",
          "   If the user did NOT supply explicit filenames for the attachment-copy, fetch the source via `ado_story` (or just the relations slice) so the plan can list filenames the user is about to inherit. Do NOT call any mutation tool yet.",
          "",
          "4. **On explicit YES, run the tools in this order:**",
          "   a. If a comment was provided: call `qa_tc_comment_add` with `workItemId: <single ID or array>` and `commentHtml: <text>`. Do NOT set `acknowledgeCrossUs` on the first call.",
          "   b. If attachments were requested: call `qa_tc_attachments_copy` with `sourceWorkItemId`, `targetTestCaseIds: <single ID or array>`, and `filenameFilter` if the user named specific files. Default `skipDuplicatesByFilename: true` is preferred — only override to `false` if the user explicitly says they want duplicates re-uploaded.",
          "",
          "5. **Handle structured `needs-confirmation` / `needs-input` responses (BOTH tools share the same precheck):**",
          "   - **`reason: \"precheck-failed\"`** (🚫 BLOCK): some IDs are not Test Cases or couldn't be fetched. Show `typeRefusals` and `fetchFailures` verbatim. Stop. Ask the user to supply a corrected ID list, then re-run from step 1 — do NOT call the tool with a filtered list on your own.",
          "   - **`reason: \"cross-us-bulk-update\"`** (⚠️ WARN — note the reason string is shared with the update tool): the targets span more than one User Story. Show the `breakdown` verbatim and ask: 'These TCs belong to N different User Stories. Is that intentional? Reply **YES** to proceed, or **no** to cancel.' On explicit YES, re-run the same tool call with `acknowledgeCrossUs: true`. Apply this independently per tool — if the comment tool flagged it but you re-ran with the ack, you must STILL handle the same flag if the attachments tool subsequently raises it.",
          "",
          "6. **Handle attachment-copy-specific responses:**",
          "   - **`reason: \"source-not-found\"`** (🚫 BLOCK): the source work item ID is wrong. Surface the message and ask the user for the correct source ID. Stop.",
          "   - **`reason: \"no-attachments-to-copy\"`** (ℹ️ NOOP): the source has no attachments (or none matched the filter). Show the message and the `availableFilenames` list. Ask whether the user wants to refine the filter or skip attachments.",
          "   - **`reason: \"all-uploads-failed\"`** (🚫 BLOCK): every source attachment failed to download/upload. Show `uploadFailures`. Stop.",
          "",
          "7. **Report the outcome.**",
          "   - If only one tool ran, surface its response (single-ID JSON or bulk markdown table) verbatim.",
          "   - If both ran, show TWO sections (`### Comment` then `### Attachments`) with each tool's response. If either was `isError: true`, lead with a single `⚠️ PARTIAL` line summarising what landed and what didn't.",
          "",
          "**Universal rules (mirror /qa-tc-update):**",
          "- NEVER call `qa_tc_comment_add` or `qa_tc_attachments_copy` without an explicit YES to the upfront plan. 'hmm', 'idk', sarcasm, frustration, rhetorical questions are NOT consent.",
          "- NEVER pass `acknowledgeCrossUs: true` without an explicit YES to the cross-US warning.",
          "- NEVER auto-retry a failed call. Surface the failure and let the user decide.",
          "- NEVER substitute one tool for the other (e.g. don't fold a comment into the attachment relation's `attributes.comment` — the user asked for both, run both).",
          "",
          CONFIRM_BEFORE_ACT_CONTRACT,
        ].join("\n"),
      },
