    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "6. If push_tc_draft_to_ado returns 'US already has N test case(s) linked in ADO' with a list: show that list to me verbatim. Ask: 'These test cases already exist on this US. Do you want to (a) UPDATE existing — needs ADO IDs in draft, use repush=true; (b) ADD NEW ones alongside existing — use insertAnyway=true; or (c) CANCEL?' Do NOT set insertAnyway=true without my explicit confirmation.",
          "7. If I provide feedback or edits: Update draft via save_tc_draft, then ask for confirmation again.",
          "8. NEVER call push_tc_draft_to_ado without explicit user confirmation (YES, approved, push, etc.). NEVER pass insertAnyway=true without showing me the existing TCs first and getting explicit approval.",
        ].join("\n"),
      },
    }],
  }));
