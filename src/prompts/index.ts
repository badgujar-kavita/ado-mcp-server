    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "",
          "Apply the QA architect methodology from .cursor/skills/draft-test-cases-salesforce-tpm/SKILL.md: analyze US + Solution Design, validate coverage matrix, add Functionality Process Flow and Test Coverage Insights at the start of the draft, then generate test cases.",
          ...
          "   - PRE-REQUISITE: Always unique per user story. Generate from US + Solution Design. Prefer Object.Field = Value; use narrative when describing scenario setup (e.g. 'Entity without X config OR record for which no mapping exists'). Use [Config should be setup/available] or [Config should be setup] as bracket hints when config is required.",
          "6. Styling rules:",
          ...
          "8. Call save_tc_draft to create the markdown. Do NOT pass planId (it will be auto-derived during push). ALWAYS pass workspaceRoot (the open folder path) or draftsPath (when user specifies a location). If user has a folder open, use workspaceRoot so drafts go to <folder>/tc-drafts/ (created if missing). If user says 'save to X' or 'create under folder Y', use draftsPath. No hardcoded path.",
          "9. After saving, present a concise summary with the markdown file link prominently displayed at the top for easy access. Include: file link (clickable), version, test case count, and key highlights (coverage checklist, process flow type [Mermaid/Text], TC breakdown by category).",
    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "3. If NO draft: Fetch US via get_user_story, apply the QA architect methodology from .cursor/skills/draft-test-cases-salesforce-tpm/SKILL.md (analyze US + Solution Design, coverage matrix, process flow, Test Coverage Insights, and derive business-specific terms from the source material instead of assuming TPM-specific language), propose test cases, then call save_tc_draft. ALWAYS pass workspaceRoot (open folder) or draftsPath (user-specified). Tell me: 'I've created a draft. Please review the draft at the path shown. When ready, run this command again and confirm.' Do NOT call push_tc_draft_to_ado.",
