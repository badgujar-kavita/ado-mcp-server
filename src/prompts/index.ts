    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "7. Call save_tc_draft to create the markdown and JSON. Do NOT pass planId (it will be auto-derived during push). ALWAYS pass workspaceRoot (the open folder path) or draftsPath (when user specifies a location). If user has a folder open, use workspaceRoot so drafts go to <folder>/tc-drafts/ (created if missing). If user says 'save to X' or 'create under folder Y', use draftsPath. No hardcoded path.",
          "8. Tell me: 'Review the draft at the path shown above. Plan ID will be auto-derived from the User Story when you push. Provide feedback for revisions, or run /ado-testforge/create_test_cases when ready to push to ADO.'",
