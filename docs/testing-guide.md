...
| `qa-publish` | Push a reviewed draft to ADO — agent guides you through confirmation at every step |
...
| `qa-tc-update` | Update a test case, or apply the same fields uniformly to many |
...
| `qa_publish_push` | Push approved draft to ADO (subfolder + legacy). **Always ensures** suite hierarchy before creating TCs (plan precedence: `planId` arg > draft planId > auto-derive). Returns `needs-input` (`plan-resolution-failed` / `sprint-resolution-failed` / `missing-fields`) when auto-derivation fails so the agent can ask for overrides and re-run. Returns `needs-confirmation` (`override-mismatch`) when an override doesn't match the US — re-run with `confirmMismatch: true` after explicit pick. Success message renders TC→ADO mappings as markdown links; when a push both updates and creates, it breaks out Updated vs. Created sections. **Publish consent gates (Phase A):** `draft-status-draft` (YES → `approveAndPush: true`), `approved-without-ids` (A → `resetToDraft: true` / B cancel), `approved-with-ids-no-repush` (A → `repush: true` / B cancel), `repush-missing-ids` (fix draft), `reset-to-draft-complete` (success, non-error). **Publish consent gates (Phase B):** `draft-ids-not-linked` (A → `proceedWithUnlinkedIds: true` / B cancel), `existing-tcs-unmapped` (⚠️ replaces the Phase A A/B/C duplicate preflight — A → `attemptMapping: true`, B → `insertAnyway: true`, C cancel), `mapping-preview` (YES → `acknowledgeMapping: true` + `userConfirmedMapping: [...]` / no cancel), `mapping-drift` (re-run `attemptMapping: true`), `tc-number-mismatch` (A cancel + fix / B `insertAnyway: true`), `extras-in-ado` (YES → `acknowledgeExtras: true` — orphans never deleted / no cancel), `mixed-update-create` (YES → `acknowledgeMixedOp: true` / no cancel). After confirmed mapping or mixed op, per-TC update-vs-create decided from in-memory `adoWorkItemId` — `repush: true` not required for these authorized flows. | `userStoryId`, `repush` (optional), `insertAnyway` (optional), `approveAndPush` (optional), `resetToDraft` (optional), `attemptMapping` (optional), `acknowledgeMapping` (optional), `userConfirmedMapping` (optional), `acknowledgeExtras` (optional), `acknowledgeMixedOp` (optional), `proceedWithUnlinkedIds` (optional), `planId` (optional override), `sprintNumber` (optional override), `confirmMismatch` (optional) |
