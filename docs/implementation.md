...
- **`qa_tc_update`** -- Update one or more fields (partial or full) on a single test case OR a batch of test cases. Pass only fields to change for partial update; pass all fields for full replace. Fields: title, description, prerequisites, steps, priority, state, assignedTo, areaPath, iterationPath.
